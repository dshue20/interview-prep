...
function pow(base, exponent) {
    ...
    const num = base * pow(base, Math.abs(exponent) - 1);
...
function flatten(data) {
    ...
    let flattened = [];
    data.forEach(ele => flattened = flattened.concat(flatten(ele)));
    return flattened;
}
...
function fileFinder(directories, targetFile) {
    ...
    for (let folder in directories){
        // console.log(folder, targetFile, folder === targetFile);
        if (folder === targetFile){
            return true;
        } else {
            found = found || fileFinder(directories[folder], targetFile);
        }
    };
    return found;
}
