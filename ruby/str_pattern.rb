...
def find_and_replace_pattern(words, pattern)
    ...
    words.each do |word|
        ...
        (0...(word.length)).each do |idx|
            if hash[pattern[idx]] 
                break if hash[pattern[idx]] != word[idx]
            else
                break if seen.include?(word[idx])
            end
            seen.add(word[idx])
            hash[pattern[idx]] = word[idx]
            result << word if idx == word.length-1
        end
    end
    result
end

p find_and_replace_pattern(["abc","deq","mee","aqq","dkd","ccc"], "abb")