...
// zeros among four hexadecimal digits and some low-case characters in the address to 
// upper-case ones, so 2001:db8:85a3:0:0:8A2E:0370:7334 is also a valid IPv6 
...
var validIPv4Address = function(ip) {
    ...
    let current = "";
    result = true;
    ip.split('').forEach((char, idx) => {
        if (result){
            // console.log(count, char);
            if (count > 3 || (!parseInt(char) & char !== 0 & char !== ".")){
                console.log('1');
                result = false;
            }
            // console.log(char);
            ...
            if (char === "."){
                if (count === 0 || parseInt(current) > 255){
                    result = false;
                }
                // console.log('hi');
                count = 0;
            ...
            if (idx === ip.length - 1){
                if (parseInt(current) > 255) result = false;
            };
        };
    });
    return result;
...
console.log(validIPv4Address("172.16.254.1"));
console.log(validIPv4Address("172.16.254.01"));
console.log(validIPv4Address("172.a.254.1"));
console.log(validIPv4Address("172.266.254.1"));